...
static inline void writeGridLine(FILE *f, int x, int y, int x2, int y2, int usage, int cap) {
    int r, g, b, width;
    if (usage == 0) {
        return;
    } else if (usage <= cap) {
...
void writeCongestionSvg(const RoutingInst &inst, const char *filename) {
    ...
    for (int y = 0; y < inst.gy; y++) {
        for (int x = 0; x < inst.gx; x++) {
            ...
            if (x < inst.gx - 1) {
                writeGridLine(f, x, y, x+1, y, cell.right.utilization, inst.cap);
            }
            if (y < inst.gy - 1) {
                writeGridLine(f, x, y, x, y+1, cell.down.utilization, inst.cap);
            }
        }
    }
